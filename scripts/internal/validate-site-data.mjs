...
for (const item of items) {
  ...
  for (const value of [item.rarity, item.max_count, item.buy_price, item.sell_price])
...
for (const quest of quests) {
  ...
  assert(
    quest.clear_condition_type === null ||
      (Number.isInteger(quest.clear_condition_type) && quest.clear_condition_type >= 0),
  );
}
