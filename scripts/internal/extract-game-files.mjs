...
const modulus = Buffer.from(
  '',
  'hex',
);
...
function decryptResource(b) {
  const m = intLE(Buffer.from('13d79c8988914810d7aa78aef859df7d3c43a0d0bb3677b5f05c02af65d8770300', 'hex')),
    e = intLE(Buffer.from('c0c2771f5b346a01c7d4d7852e422b3b163a171316ea833030df3ff42593200100', 'hex'));
  const out = Buffer.alloc(Math.floor((b.length - 8) / 128) * 8);
  for (let p = 8, j = 0; p + 128 <= b.length; p += 128, j += 8) {
  ...
  if (n > out.length) throw Error('Resource size');
...
function hash(s) {
  ...
  k = 0;
  for (let j = i; j < b.length; j++) k |= b[j] << ((j - i) * 8);
  if (i < b.length) h ^= Math.imul(rot(Math.imul(k, 0xcc9e2d51), 15), 0x1b873593);
  h ^= b.length;
...
const filter = new RegExp(
  process.argv[3] ||
    'GameDesign/.*(Reward|SupplyItem|Gather|Collect|GimmickBasic|GimmickText|GmID|ItemData).*\\.user\\.3$',
  'i',
);
...
const names = new Map(
  fileList
    ...
    .map((s) => [`${hash(s.toUpperCase())}:${hash(s.toLowerCase())}`, s]),
);
const game = process.argv[2];
const manifestPath = path.join(root, 'manifest.json');
const records = new Map(
  !process.argv.includes('--fresh') && fs.existsSync(manifestPath)
    ? Object.entries(JSON.parse(fs.readFileSync(manifestPath, 'utf8')))
    : [],
);
const archives = fs
  .readdirSync(game)
  .filter((s) => s.endsWith('.pak'))
  .sort((a, b) => {
    const p = (s) => Number(s.match(/patch_(\d+)/)?.[1] || 0);
    return p(a) - p(b) || a.localeCompare(b);
  });
function read(fd, len, pos) {
  const b = Buffer.alloc(len);
  if (fs.readSync(fd, b, 0, len, pos) !== len) throw Error('Short read');
  return b;
}
for (const archive of archives) {
  ...
  try {
    ...
    if (h.toString('ascii', 0, 4) !== 'KPKA' || h[4] !== 4) throw Error('Header');
    const flags = h.readUInt16LE(6),
      count = h.readUInt32LE(8);
    if (flags & ~40) throw Error(`Unsupported flags ${flags}`);
    ...
    if (flags & 8) {
      ...
      for (let i = 0; i < toc.length; i++) toc[i] ^= (i + key[i % 32] * key[i % 29]) & 255;
    }
    const chunks = [];
    let block = 0;
    if (flags & 32) {
      const pos = 16 + toc.length + (flags & 8 ? 128 : 0),
        ch = read(fd, 8, pos);
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!name) continue;
      const offset = Number(toc.readBigUInt64LE(p + 8)),
        size = Number(toc.readBigUInt64LE(p + 16)),
        expanded = Number(toc.readBigUInt64LE(p + 24)),
        attr = toc.readUInt32LE(p + 32);
      let data;
      if (attr & 0x1000000) {
        const parts = [];
        for (let j = 0; j < Math.ceil(expanded / block); j++) {
          ...
          if (c.size !== block) b = zstdDecompressSync(b);
          if (b.length !== block) throw Error('Chunk length');
          parts.push(b);
        }
        data = Buffer.concat(parts).subarray(0, expanded);
      } else {
        data = read(fd, size, offset);
        if (attr & 0xff0000) data = decryptResource(data);
        if ((attr & 15) === 1) data = inflateRawSync(data);
        else if ((attr & 15) === 2) data = zstdDecompressSync(data);
        else if ((attr & 15) !== 0) throw Error('Compression');
      }
      if (data.length !== expanded) throw Error('Expanded size mismatch');
      const out = path.resolve(root, 'raw', name);
      if (!out.startsWith(path.resolve(root, 'raw') + path.sep)) throw Error('Unsafe name');
