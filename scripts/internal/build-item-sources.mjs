...
const stages = new Map(
  read(path.join(dataDirectory, 'lookups.json')).stages.map((stage) => [stage.game_id, stage.names.ja]),
);
...
function add(itemId, source, origin) {
  ...
  if (!sources[itemId]) sources[itemId] = [];
...
function resolveText(value, depth = 0) {
  ...
  return value
    .replace(/<REF Item_IT_(\d+)>/g, (_, id) => items.get(Number(id))?.names.ja ?? '')
...
const gimmickNames = new Map(
  rows('Common/Gimmick/GimmickTextData.user.3').map((row) => [row._GimmickId, resolveText(texts[row._Name]?.ja)]),
);
  ...
  for (const graph of scene.instances.filter((row) => row?.$type === 'via.pointgraph.PointGraph')) {
    ...
    for (let i = 0; i < list.readUInt32LE(8); i++) {
      ...
      for (const index of data.objects) {
        ...
        if (!entries.some((entry) => entry.stageId === stage._FixedID))
          entries.push({ stageId: stage._FixedID, origin: name });
...
for (const row of rows('Facility/ItemShopData.user.3').filter((entry) => entry._ItemId > 0)) {
  add(
    row._ItemId,
    { location: '物資補給所', method: '購入', condition: row._StoryPackage >= 0 ? 'ストーリー進行で解放' : undefined },
    ['Facility/ItemShopData.user.3'],
  );
}
...
for (const row of collection.instances.filter((entry) => entry?.$type === 'app.user_data.CollectionItemData.cData')) {
  ...
  const table = numTypes.find(
    (entry) => deref(tables, entry._NumType)._Value === deref(collection, row._NumType)._Value,
  );
  ...
  for (const stage of stageEnums.filter((entry) => /^ST10[1-5]$/.test(entry._EnumName))) {
    ...
    for (const [key, label] of [
      ...
      const quantity = table._Table
        .map((ref) => deref(tables, ref))
        .find((entry) => deref(tables, entry._Level)._Value === level);
      if (!quantity) throw new Error('Missing collection level');
      if (
        quantity._Data.some((ref) => {
          const entry = deref(tables, ref);
          return entry._Num > 0 && entry._Probability > 0;
        })
      )
    ...
    if (conditions.length)
      add(
        itemId,
        {
          location: stages.get(stage._FixedID),
          method: '素材採集依頼',
          condition: conditions.length === 3 ? 'ストーリー進行で解放' : conditions.join('・'),
        },
        ['Facility/CollectionItemData.user.3', 'Facility/CollectionTable.user.3'],
      );
  }
}
...
for (const name of report.ok.filter((entry) => entry.endsWith('_MsData.user.3'))) {
  ...
  if (title && !title.includes('<'))
    missionNames.set(deref(data, mission._MissionIDSerial)._Value, { title, origin: name });
}
...
for (const row of settings.instances.filter((entry) => entry?._missionID)) {
  ...
  for (const [tableId, data, method] of [
    ...
    if (!tableId) continue;
    for (const reward of data.instances.filter(
      (entry) => entry?._tableId === tableId && entry._num > 0 && entry._probability > 0,
    )) {
      add(
        deref(data, reward._itemId)._Value,
        {
          location: quest.title,
          method,
          amount: reward._num,
          chance: reward._probability,
          condition: '報酬 1 枠あたり',
        },
        [
          quest.origin,
          'Mission/_UserData/_Reward/QuestRewardSetting.user.3',
          method === 'クエスト報酬'
            ? 'Mission/_UserData/_Reward/CommonRewardData.user.3'
            : 'Mission/_UserData/_Reward/AddRewardData.user.3',
        ],
      );
    }
  }
...
for (const row of missionRewards.instances.filter((entry) => entry?._MissonId)) {
  ...
  row._ItemId.forEach((ref, index) => {
    if (row._ItemNum[index] > 0)
      add(
        deref(missionRewards, ref)._Value,
        { location: mission.title, method: 'ミッション報酬', amount: row._ItemNum[index] },
        [mission.origin, 'Mission/_UserData/_Reward/MissionRewardData.user.3'],
      );
  });
}
for (const row of rows('Mission/_UserData/SupplyItemData.user.3').filter(
  (entry) => entry._ItemId > 0 && entry._ItemCount > 0,
)) {
  add(
    row._ItemId,
    { location: stages.get(row._Stage) ?? 'クエスト', method: '支給品', condition: 'クエスト・支給条件による' },
    ['Mission/_UserData/SupplyItemData.user.3'],
  );
}
for (const name of report.ok.filter((entry) => /Common\/Enemy\/EM[15]\d{3}_\d{2}_\d\.user\.3$/.test(entry))) {
  ...
  for (const row of data.instances.filter((entry) => entry?.$type === 'app.user_data.EnemyRewardData.cData')) {
    if (row._probabilityStory > 0 && row._RewardNumStory > 0)
      add(
        deref(data, row._IdStory)._Value,
        {
          location: title,
          method: 'モンスター報酬',
          rank: '下位',
          amount: row._RewardNumStory,
          chance: row._probabilityStory,
        },
        [name],
      );
    row._IdEx.forEach((ref, index) => {
      if (row._probabilityEx[index] > 0 && row._RewardNumEx[index] > 0)
        add(
          deref(data, ref)._Value,
          {
            location: title,
            method: 'モンスター報酬',
            rank: '上位',
            amount: row._RewardNumEx[index],
            chance: row._probabilityEx[index],
          },
          [name],
        );
    });
...
for (const row of bonus.instances.filter((entry) => entry?._ItemId)) {
  row._ItemId.forEach((ref, index) => {
    if (row._ItemNum[index] > 0 && row._Probability[index] > 0)
      add(
        deref(bonus, ref)._Value,
        {
          location: '素材採集依頼',
          method: '追加報酬',
          condition: '依頼先・設備の発展状況による',
          amount: row._ItemNum[index],
        },
        ['Facility/CollectionBonusItemData.user.3'],
      );
  });
}
for (const row of rows('Facility/ItemExchange.user.3').filter(
  (entry) => entry._RewardItemId > 0 && entry._RewardNum > 0,
)) {
  const payment = items.get(row._PayItemId)?.names.ja;
  if (payment)
    add(
      row._RewardItemId,
      { location: 'アイテム交換', method: `${payment} ${row._PayNum} 個と交換`, amount: row._RewardNum },
      ['Facility/ItemExchange.user.3'],
    );
}
for (const row of rows('Facility/BarterData.user.3').filter((entry) => entry._Request > 0 && entry._MaxCount > 0)) {
  ...
  row._Reward.forEach((itemId, index) => {
    if (row._RewardNum[index] > 0)
      add(
        itemId,
        {
          location: [stage, npc].filter(Boolean).join(' / '),
          method: `もちもの交換: ${payment} ${row._RequestNum} 個`,
          amount: row._RewardNum[index],
          condition: 'ストーリー進行・環境により品揃えが変化',
        },
        ['Facility/BarterData.user.3'],
      );
  });
}
const workshop = load('Facility/LargeWorkshopResultData.user.3');
for (const row of workshop.instances.filter(
  (entry) => entry?.$type === 'app.user_data.LargeWorkshopResultData.cResultItemData',
)) {
  add(
    deref(workshop, row._ItemId)._Value,
    { location: '油涌き谷 / 大窯', method: '精錬', condition: '精錬の結果による' },
    ['Facility/LargeWorkshopResultData.user.3'],
  );
}
for (const row of rows('Common/Item/FixItems.user.3').filter((entry) => entry._ItemId > 0)) {
  add(
    row._ItemId,
    {
      location: '基本装備',
      method: '常備アイテム',
      condition: row._StoryPackage >= 0 ? 'ストーリー進行で解放' : undefined,
    },
    ['Common/Item/FixItems.user.3'],
  );
}
for (const row of rows('Facility/GrillData.user.3').filter((entry) => entry._RewardItemId)) {
  ...
  row._RewardItemId.forEach((id, index) => {
    if (row._RewardItemNum[index] > 0 && row._RewardItemRate[index] > 0)
      add(
        id,
        {
          location: '焚き火焼き',
          method: `${material}を焼く`,
          amount: row._RewardItemNum[index],
          chance: row._RewardItemRate[index],
          condition: '追加報酬 1 枠あたり',
        },
        ['Facility/GrillData.user.3'],
      );
  });
}
for (const row of rows('Facility/MakaData.user.3').filter((entry) => entry._ItemId > 0)) {
  add(
    row._ItemId,
    { location: 'マカ錬金', method: '錬金', condition: `${row._NeedPoint} ポイント / ストーリー進行で解放` },
    ['Facility/MakaData.user.3'],
  );
}
for (const row of rows('Facility/SupportShipData.user.3').filter(
  (entry) => entry._ItemId > 0 && entry._StockNum > 0 && entry._Rate > 0,
)) {
  add(row._ItemId, { location: '補給船', method: '購入', condition: `${row._Point} ポイント / 品揃えによる` }, [
    'Facility/SupportShipData.user.3',
  ]);
}
for (const [name, method] of [
  ['MoriverSwopReward', '交換'],
  ['MoriverSharingData', 'おすそわけ'],
]) {
  for (const row of rows(`Facility/${name}.user.3`).filter(
    (entry) => entry._ItemId > 0 && entry._ItemNum > 0 && entry._Rate > 0,
  )) {
...
for (const row of bounties.instances.filter((entry) => entry?._BountyName)) {
  ...
  row._Reward.forEach((ref, index) => {
    if (row._RewardNum[index] > 0)
      add(deref(bounties, ref)._Value, { location: title, method: '依頼報酬', amount: row._RewardNum[index] }, [
        'Facility/BountyData.user.3',
      ]);
  });
}
for (const source of Object.values(sources).flat()) {
  if (!source.location || !source.method || source.location.includes('<') || source.method.includes('<'))
    throw new Error('Invalid source label');
  if (source.amount !== undefined && (!Number.isFinite(source.amount) || source.amount <= 0))
    throw new Error('Invalid source quantity');
  if (source.chance !== undefined && (!Number.isFinite(source.chance) || source.chance <= 0 || source.chance > 100))
    throw new Error('Invalid source probability');
}
...
const monsterRewards = new Set(
  read(path.join(dataDirectory, 'monsters.json')).flatMap((monster) => monster.rewards.map((reward) => reward.item_id)),
);
const missing = [...items.values()]
  .filter((item) => !item.recipes.length && !monsterRewards.has(item.game_id) && !sources[item.game_id])
  .map((item) => ({ id: item.game_id, name: item.names.ja }));
fs.writeFileSync(path.join(input, 'missing-sources.json'), JSON.stringify(missing, null, 2));
console.log(
  `入手先を生成: ${Object.keys(sources).length} アイテム、${Object.values(sources).flat().length} 件（入手先未特定: ${missing.length} アイテム）`,
);
