...
const items = rows(load('Common/Item/itemData'), 'app.user_data.ItemData.cData')
  ...
  .map((row) => ({
    game_id: row._ItemId,
    names: localized(row._RawName),
    descriptions: localized(row._RawExplain),
    kind: itemKinds[row._Type],
    rarity: 19 - row._Rare,
    max_count: row._MaxCount,
    sell_price: row._SellPrice,
    buy_price: row._BuyPrice,
    recipes: [],
    out_box: row._OutBox,
  }));
...
function use(itemId, value) {
  ...
  if (!itemUses[itemId]) itemUses[itemId] = [];
...
for (const row of rows(load('Common/Item/ItemRecipe'), 'app.user_data.cItemRecipe.cData')) {
  ...
  for (const id of new Set(materials))
    use(id, { category: '調合', name: item.names.ja, amount: materials.filter((value) => value === id).length });
}
...
const stages = stageEnums
  .filter((row) => stageTextKeys[row._EnumName])
  .map((row) => ({
    game_id: row._FixedID,
    names: { ja: resolve(byName.get(`RefEnvironment_${stageTextKeys[row._EnumName]}`)) },
    bitmask_value: 2 ** (row._EnumValue + 1),
  }));
if (stages.some((stage) => !stage.names.ja)) throw Error('フィールド名がありません');
const species = rows(load('Common/Enemy/EnemySpecies'), 'app.user_data.EnemySpeciesData.cData').map((row) => ({
  game_id: row._EmSpecies,
  names: localized(row._EmSpeciesName),
}));
const speciesById = new Map(species.map((row) => [row.game_id, row.names.ja]));
const partNames = rows(load('Common/Enemy/EnemyPartsTypeData'), 'app.user_data.EnemyPartsTypeData.cData').map(
  (row) => ({ game_id: row._EmPartsType, part: String(row._EmPartsType), names: localized(row._EmPartsName) }),
);
const partIds = new Set(partNames.map((row) => row.part));
const enumNames = new Map(
  load('Enemy/CommonData/EnumMaker/EmID')
    .instances.filter((row) => row?._EnumName)
    .map((row) => [row._FixedID, row._EnumName]),
);
const enemyData = new Map(
  rows(load('Common/Enemy/EnemyData'), 'app.user_data.EnemyData.cData').map((row) => [row._enemyId, row]),
);
const sizes = new Map(
  rows(load('Enemy/CommonData/Data/EmCommonSize'), 'app.user_data.EmParamSize.cSizeData').map((row) => [
    row._EmId,
    row,
  ]),
);
...
const presetByGuid = new Map(
  presets.instances.filter((row) => row?._InstanceGuid).map((row) => [row._InstanceGuid, row]),
);
const conditionsById = new Map(
  rows(badConditions, 'app.user_data.EmParamBadCondition2.EnemyBadConditionSetting').map((row) => [row._EmId, row]),
);
const elements = ['fire', 'water', 'thunder', 'ice', 'dragon'];
const statusFields = {
  ParalyzePriset: 'paralysis',
  PoisonPriset: 'poison',
  SleepPriset: 'sleep',
  StunPriset: 'stun',
  BlastPreset: 'blastblight',
  StaminaPreset: 'exhaust',
  FlashPriset: 'flash',
  EarPriset: 'noise',
};
...
const rewardKinds = {
  ...
  7: 'broken-part',
  8: 'wound-destroyed',
  911862272: 'carve-rotten',
  '-2122632576': 'carve-rotten-severed',
  '-1024798784': '歴戦傷口破壊',
};
const monsters = [];
function buildMonster({ enemyId, stageBits, boss }) {
  ...
  const parts = rows(data, 'app.user_data.EmParamParts.cParts').map((row) => {
    ...
    return {
      part,
      base_health: ref(data, row._Vital[0])?._Value ?? null,
      ...
      multipliers: Object.fromEntries(
        Object.entries({
          ...
          ice: '_Ice',
          dragon: '_Dragon',
        }).map(([key, field]) => [key, meat[field] / 100]),
      ),
    };
  });
  ...
  for (const row of rows(rewardsData, 'app.user_data.EnemyRewardData.cData')) {
    ...
    if (type !== 10) {
      kind = rewardKinds[type] ?? 'monster-reward';
      part = type === 7 ? breakParts.get(row._partsIndex) : undefined;
    }
    if (!kind) continue;
    const values = [
      ['low', scalar(rewardsData, row._IdStory), row._RewardNumStory, row._probabilityStory],
  ...
  if (boss) {
    ...
    weaknesses.push(
      ...attributeOrder
        .filter((_, index) => weaknessBits & (1 << (index + 1)))
        .map((element) => ({ kind: 'element', element })),
    );
    resistances.push(
      ...elements
        .filter((element) => parts.every((part) => part.multipliers[element] === 0))
        .map((element) => ({ kind: 'element', element })),
    );
    ...
    for (const [field, name] of Object.entries(statusFields)) {
      ...
      const entry = {
        kind: ['flash', 'noise'].includes(name) ? 'effect' : 'status',
        [['flash', 'noise'].includes(name) ? 'effect' : 'status']: name,
      };
      if (!preset) {
        if (guid && guid !== '00000000-0000-0000-0000-000000000000')
          throw Error(`状態異常プリセットがありません: ${code}/${name}`);
  ...
  monsters.push({
    game_id: enemyId,
    names: localized(enemy._EnemyName),
    species: speciesById.get(enemy._Species),
    descriptions: localized(enemy._EnemyExp),
    features: localized(enemy._EnemyFeatures),
    tips: localized(enemy._EnemyTips),
    variants: [],
    base_health: Number.isFinite(settings?._BaseHealth) ? settings._BaseHealth : null,
    size: size
      ? {
          base: size._BaseSize,
          mini: (size._BaseSize * size._CrownSize_Small) / 100,
          silver: (size._BaseSize * size._CrownSize_Big) / 100,
          gold: (size._BaseSize * size._CrownSize_King) / 100,
        }
      : { base: null, mini: null, silver: null, gold: null },
...
const questCategory = (file) => {
  ...
  if (
    ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '700', '720', '730', '740'].includes(
      prefix,
    )
  )
    return '任務';
...
for (const file of questFiles) {
  ...
  for (const row of rows(data, 'app.user_data.QuestData')) {
    ...
    const manualObjective =
      clearConditionMessage && !clearConditionMessage._IsAuto ? resolve(texts[clearConditionMessage._MsgID]?.ja) : '';
    const targetMonsters = (clearCondition?._TargetInfoArray ?? []).flatMap((targetRef) => {
      ...
      const nameGuid =
        target._LegendaryID === 1
          ? monster._EnemyLegendaryName
          : target._LegendaryID === 2
            ? monster._EnemyLegendaryKingName
            : monster._EnemyName;
      return [
        {
          game_id: monster._enemyId,
          names: localized(texts[nameGuid]?.ja ? nameGuid : monster._EnemyName),
          amount: target._TargetValue,
        },
      ];
    });
...
const valueList = (data, value) =>
  Array.isArray(value) ? value.map((entry) => scalar(data, entry)) : scalar(data, value);
...
const skillCommon = rows(skillCommonData, 'app.user_data.SkillCommonData.cData')
  ...
  .map((row) => ({
    game_id: row._skillId,
    names: localized(row._skillName),
    descriptions: localized(row._skillExplain),
    type: row._skillType,
    category: row._skillCategory,
    icon: row._SkillIconType,
  }));
const skillById = new Map(skillCommon.map((skill) => [skill.game_id, skill]));
const skillLevels = rows(skillData, 'app.user_data.SkillData.cData')
  .filter((row) => skillById.has(row._skillId))
  .map((row) => ({
    skill_id: row._skillId,
    level: row._SkillLv,
    names: localized(row._skillName),
    descriptions: localized(row._skillExplain),
    values: row._value,
    unlocks: row._openSkill,
  }));

const armorData = load('Common/Equip/ArmorData');
const armorByKey = new Map(
  rows(armorData, 'app.user_data.ArmorData.cData').map((row) => [
    `${scalar(armorData, row._Series)}:${scalar(armorData, row._PartsType)}`,
    row,
  ]),
);
const armorSeriesData = load('Common/Equip/ArmorSeriesData');
const armorSeries = rows(armorSeriesData, 'app.user_data.ArmorSeriesData.cData')
  .filter((row) => texts[row._Name]?.ja)
  .map((row) => ({
    game_id: scalar(armorSeriesData, row._Series),
    names: localized(row._Name),
    rarity: scalar(armorSeriesData, row._Rare),
    price: row._Price,
    one_set: row._IsOneSet,
  }));
...
const armorUpgrades = rows(armorUpgradeData, 'app.user_data.ArmorUpgradeData.cData').map((row) => ({
  rarity: scalar(armorUpgradeData, row._Rare),
  story_no: scalar(armorUpgradeData, row._StoryNo),
  max_level: row._MaxLevel,
  defense_increment: row._DefUpValue,
  points: row._Point,
  price: row._Price,
  special: row._IsSpecialUpgrade,
}));
const armorUpgradeRecipeData = load('Common/Equip/ArmorUpgradeRecipeData');
const armorUpgradeRecipes = rows(armorUpgradeRecipeData, 'app.user_data.ArmorUpgradeRecipeData.cData')
  .map((row) => ({
    series_id: row._SeriesId,
    materials: row._Item.flatMap((id, index) =>
      id > 1 && row._ItemNum[index] > 0 ? [{ item_id: id, amount: row._ItemNum[index] }] : [],
    ),
  }))
  .filter((recipe) => recipe.materials.length);
const armor = rows(armorData, 'app.user_data.ArmorData.cData')
  .filter((row) => texts[row._Name]?.ja)
  .map((row) => {
    const series_id = scalar(armorData, row._Series);
    const part = scalar(armorData, row._PartsType);
    const skills = valueList(armorData, row._Skill)
      .map((skillId, index) => ({ skill_id: skillId, level: row._SkillLevel[index] }))
      .filter((skill) => skill.skill_id !== 0);
    return {
      game_id: `armor:${series_id}:${part}`,
      series_id,
      part,
      names: localized(row._Name),
      descriptions: localized(row._Explain),
      rarity: armorSeriesById.get(series_id)?.rarity ?? null,
      price: armorSeriesById.get(series_id)?.price ?? null,
      defense: row._Defense,
      resistances: row._Resistance,
      slots: valueList(armorData, row._SlotLevel),
      skills,
    };
  });
...
for (const row of rows(armorRecipeData, 'app.user_data.ArmorRecipeData.cData')) {
  ...
  const materials = row._Item.flatMap((id, index) =>
    id > 1 && row._ItemNum[index] > 0 ? [{ item_id: id, amount: row._ItemNum[index] }] : [],
  );
  if (item && materials.length)
    armorRecipeList.push({
      armor_id: `armor:${row._SeriesId}:${row._PartsType}`,
      series_id: row._SeriesId,
      part: row._PartsType,
      materials,
      key_item_id: row._KeyItemId,
      key_enemy_id: row._KeyEnemyId,
      key_story_no: row._KeyStoryNo,
      hunter_rank: row._FlagHunterRank,
      time_rank: scalar(armorRecipeData, row._TimeRank),
    });
  for (let i = 0; i < row._Item.length; i++)
    if (row._Item[i] > 1 && row._ItemNum[i] > 0) {
...
const amulets = new Map(
  rows(amuletData, 'app.user_data.AmuletData.cData').map((row) => [
    `${scalar(amuletData, row._AmuletType)}:${row._Lv}`,
    row,
  ]),
);
const amuletList = [...amulets]
  .filter(([, row]) => texts[row._Name]?.ja)
  .map(([key, row]) => {
    const skills = valueList(amuletData, row._Skill)
      .map((skillId, index) => ({ skill_id: skillId, level: row._SkillLevel[index] }))
      .filter((skill) => skill.skill_id !== 0);
    return {
      game_id: `amulet:${key}`,
      amulet_type: scalar(amuletData, row._AmuletType),
      level: row._Lv,
      names: localized(row._Name),
      descriptions: localized(row._Explain),
      rarity: scalar(amuletData, row._Rare),
      price: row._Price,
      skills,
    };
  });
...
for (const row of rows(amuletRecipeData, 'app.user_data.AmuletRecipeData.cData')) {
  ...
  const materials = row._ItemId.flatMap((id, index) =>
    id > 1 && row._ItemNum[index] > 0 ? [{ item_id: id, amount: row._ItemNum[index] }] : [],
  );
  if (item && materials.length)
    amuletRecipeList.push({
      amulet_id: `amulet:${row._AmuletType}:${row._Lv}`,
      amulet_type: row._AmuletType,
      level: row._Lv,
      materials,
      key_item_id: row._KeyItemId,
      key_enemy_id: row._KeyEnemyId,
  ...
  for (let i = 0; i < row._ItemId.length; i++)
    if (row._ItemId[i] > 1 && row._ItemNum[i] > 0) {
...
for (const [code, category] of Object.entries(weapons)) {
  ...
  for (const row of rows(load(`Common/Weapon/${code}Recipe`), 'app.user_data.WeaponRecipeData.cData')) {
    ...
    if (weaponId > 0 && materials.length && weaponName && !weaponName.startsWith('#Rejected#', 0))
      weaponRecipeList.push({
        weapon_id: `weapon:${code}:${weaponId}`,
        weapon_type: code,
        materials,
        key_item_id: row._KeyItemId,
        key_enemy_id: row._KeyEnemyId,
  ...
  for (const row of weaponRows.filter((entry) => {
    ...
    const details =
      code === 'Rod'
        ? { rod_insect_level: scalar(weaponData, row._RodInsectLv) }
        : ['SlashAxe', 'ChargeAxe'].includes(code)
          ? { phial_type: scalar(weaponData, row._Wp08BinType), phial_value: row._Wp08BinValue }
          : code === 'Whistle'
            ? {
                melody_type: scalar(weaponData, row._Wp05UniqueType),
                high_frequency: scalar(weaponData, row._Wp05MusicSkillHighFreqType),
                echo_type: scalar(weaponData, row._Wp05HibikiSkillType),
              }
            : code === 'GunLance'
              ? {
                  shell_type: scalar(weaponData, row._Wp07ShellType),
                  shell_level: scalar(weaponData, row._Wp07ShellLv),
                  shell_count: row._ShellNum,
                  rapid_shell_count: row._RapidShellNum,
                }
              : ['Bow', 'HeavyBowgun', 'LightBowgun'].includes(code)
                ? {
                    coating_flags: row._isLoadingBin,
                    special_ammo: scalar(weaponData, row._Wp13SpecialAmmo),
                    ammo_strength: scalar(weaponData, row._AmmoStrength),
                  }
                : {};
    const skills = valueList(weaponData, row._Skill)
      .map((skillId, index) => ({ skill_id: skillId, level: row._SkillLevel[index] }))
      .filter((skill) => skill.skill_id !== 0);
    weaponList.push({
      game_id: `weapon:${code}:${weaponId}`,
      weapon_type: code,
      category,
      names: localized(row._Name),
      descriptions: localized(row._Explain),
      rarity: 19 - scalar(weaponData, row._Rare),
      price: row._Price,
      attack: row._Attack,
      defense: row._Defense,
      affinity: row._Critical,
      attribute: scalar(weaponData, row._Attribute),
      attribute_value: row._AttributeValue,
      sub_attribute: scalar(weaponData, row._SubAttribute),
      sub_attribute_value: row._SubAttributeValue,
      slots: valueList(weaponData, row._SlotLevel),
      sharpness: row._SharpnessValList,
      handicraft: row._TakumiValList,
      skills,
      details,
    });
  }
  ...
  for (const row of nodes)
    weaponTreeList.push(
      ...row._NextDataGuidList
        .map((guid) => nodeByGuid.get(guid))
        .filter(Boolean)
        .map((next) => ({
          weapon_type: code,
          parent_id: `weapon:${code}:${row._WeaponID}`,
          child_id: `weapon:${code}:${next._WeaponID}`,
        })),
    );
}
...
const artianWeaponIds = [
  ...new Set(
    rows(artianCreateData, 'app.user_data.ArtianCreateTypeData.cData').flatMap((row) =>
      Object.entries(row)
        .filter(([key, value]) => key.startsWith('_') && weaponIds.has(`weapon:${key.slice(1)}:${value}`))
        .map(([key, value]) => `weapon:${key.slice(1)}:${value}`),
    ),
  ),
];
...
const artianBonuses = rows(artianBonusData, 'app.user_data.ArtianBonusData.cData').map((row) => ({
  id: scalar(artianBonusData, row._BonusId),
  name: localized(row._Name).ja,
  normalMax: row._GrindingMaxNum,
  gogmaMax: row._Em0078_GrindingMaxNum,
  attack: row._Attack,
  affinity: row._Critical,
  attribute: row._AttributeValue,
  sharpness: row._SharpnessValue,
}));
const decorations = rows(accessoryData, 'app.user_data.AccessoryData.cData')
  .filter((row) => texts[row._Name]?.ja)
  .map((row) => {
    const skills = valueList(accessoryData, row._Skill)
      .map((skillId, index) => ({ skill_id: skillId, level: row._SkillLevel[index] }))
      .filter((skill) => skill.skill_id !== 0);
    return {
      game_id: scalar(accessoryData, row._AccessoryId),
      names: localized(row._Name),
      descriptions: localized(row._Explain),
      type: scalar(accessoryData, row._AccessoryType),
      rarity: scalar(accessoryData, row._Rare),
      price: row._Price,
      required_slot: scalar(accessoryData, row._SlotLevelAcc),
      skills,
    };
  });
const accessoryJudgeData = load('Common/Equip/AccessoryJudgeData');
const decorationProbabilities = rows(accessoryJudgeData, 'app.user_data.AccessoryJudgeData.cData').map((row) => ({
  accessory_id: scalar(accessoryJudgeData, row._AccessoryId),
  probabilities: row._Probability,
}));
const kinsectData = load('Common/Weapon/RodInsectData');
const kinsects = rows(kinsectData, 'app.user_data.RodInsectData.cData')
  .filter((row) => texts[row._Name]?.ja)
  .map((row) => ({
    game_id: scalar(kinsectData, row._Id),
    names: localized(row._Name),
    rarity: scalar(kinsectData, row._Rare),
    price: row._Price,
    hit_type: scalar(kinsectData, row._HitType),
    dust_type: scalar(kinsectData, row._DustType),
    insect_skill: scalar(kinsectData, row._InsectSkillType),
    ground_skill: scalar(kinsectData, row._InsectSkillGType),
    attack: row._Attack,
    move_speed: row._MoveSpeed,
    recovery: row._RecoveryValue,
    stamina_recovery: row._StaminaRecoverySpeed,
    powder_level: row._PowderLevel,
  }));
for (const row of rows(load('Common/Equip/RodInsectRecipeData'), 'app.user_data.RodInsectRecipeData.cData')) {
  ...
  if (materials.length)
    kinsectRecipeList.push({
      kinsect_id: `kinsect:${row._ID}`,
      materials,
      key_item_id: row._KeyItemId,
      key_enemy_id: row._KeyEnemyId,
      key_story_no: row._KeyStoryNo,
      hunter_rank: row._FlagHunterRank,
      previous_id: row._PrevID,
    });
}
for (const item of items)
  if (!item.kind || !item.names.ja || item.rarity < 1 || item.rarity > 18)
    throw Error(`アイテムデータが不正です: ${item.game_id}`);
for (const monster of monsters)
  if (!monster.names.ja || !monster.species || !monster.parts.length)
    throw Error(`モンスターデータが不正です: ${monster.game_id}`);
fs.mkdirSync(output, { recursive: true });
for (const [file, data] of Object.entries({
  items,
  monsters,
  quests,
  lookups: { stages, species, partNames },
  'item-uses': itemUses,
}))
  fs.writeFileSync(path.join(output, `${file}.json`), JSON.stringify(data));
...
fs.writeFileSync(
  ...
  JSON.stringify({
    ...
    dataOrigin: 'Monster Hunter Wilds ゲームデータ',
    gameFiles,
    supplementalSources: [
...
for (const [file, data] of Object.entries({
  armor,
  'armor-series': armorSeries,
  'armor-upgrades': armorUpgrades,
  'armor-upgrade-recipes': armorUpgradeRecipes,
  'armor-recipes': armorRecipeList,
  amulets: amuletList,
  'amulet-recipes': amuletRecipeList,
  weapons: weaponList,
  'weapon-recipes': weaponRecipeList,
  'weapon-trees': weaponTreeList,
  decorations,
  'decoration-probabilities': decorationProbabilities,
  skills: skillCommon,
  'skill-levels': skillLevels,
  'artian-skill-data': { weaponIds: artianWeaponIds, skillPairs: artianSkillPairs, bonuses: artianBonuses },
  kinsects,
  'kinsect-recipes': kinsectRecipeList,
}))
  fs.writeFileSync(path.join(output, `${file}.json`), JSON.stringify(data));
console.log(
  `直接抽出: ${items.length} アイテム、${monsters.length} モンスター、${quests.length} クエスト、武器 ${weaponList.length} 件、防具 ${armor.length} 件、護石 ${amuletList.length} 件、装飾品 ${decorations.length} 件、スキル ${skillCommon.length} 件`,
);
