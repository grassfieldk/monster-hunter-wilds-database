...
function sourceGroup(method: string) {
  ...
  if (method.startsWith('もちもの交換: ') || method.endsWith('と交換') || method === '交換' || method === 'おすそわけ')
    return '交換・おすそわけ';
...
export function ItemSources({ item }: { item: Item }) {
  ...
  const { supportedMethods, groups } = useMemo(() => {
    ...
    for (const source of itemSources[String(item.game_id)] ?? []) {
      ...
      const material =
        source.method.startsWith('もちもの交換: ') || source.method.endsWith('と交換')
          ? source.method.replace(/^もちもの交換: /u, '').replace(/と交換$/u, '')
          : group === '焚き火焼き'
            ? source.method.replace(/を焼く$/u, '')
            : undefined;
      const questId = ['クエスト報酬', 'ミッション報酬'].includes(source.method)
        ? questIdByName.get(source.location)
        : undefined;
      ...
      add(group, {
        ...
        questId,
        monsterId,
        method: material && group === '交換・おすそわけ' ? 'もちもの交換' : source.method,
      });
    }
    for (const monster of monsters) {
      for (const reward of monster.rewards) {
        ...
        add(sourceGroup(label(reward.kind)), {
          location: text(monster.names),
          monsterId: monster.game_id,
          method: label(reward.kind),
          rank: label(reward.rank),
    ...
    const orderedMethods = [
      ...sourceOrder,
      ...[...methods].filter((method) => method !== '調合' && !sourceOrder.includes(method)),
    ];
    const groups = new Map<string, SourceRow[]>(
      orderedMethods.filter((method) => methods.has(method)).map((method) => [method, sourceRows.get(method) ?? []]),
    );
  ...
  const itemLink = (id: number) => {
    ...
    return material ? (
      <Anchor component={Link} to={`/items/${id}`}>
        {text(material.names)}
      </Anchor>
    ) : (
      `ID ${id}`
  ...
  const materialLink = (value: string) => {
    ...
    return (
      <>
        {material ? itemLink(material.game_id) : name}
        {match && ` x${match[2]}`}
      </>
    );
  };
          ...
          {item.recipes.length ? (
            <Stack gap={4}>
              {item.recipes.map((recipe, index) => (
                <Group key={index} justify="space-between" wrap="nowrap" gap="sm">
                  <Box>
                    {recipe.inputs.map((id, inputIndex) => (
                      <Fragment key={`${id}-${inputIndex}`}>
                        {inputIndex > 0 && ' + '}
                        {itemLink(id)}
                      </Fragment>
                    ))}
      ...
      {[...groups].map(([group, rows]) => {
        ...
        const locationTitle =
          group === '購入'
            ...
            : group === '採取'
              ? '採取場所'
              : group === '報酬' || group === '交換・おすそわけ'
                ? '入手先'
                : rows.some((row) => row.monsterId !== undefined) || group === 'モンスター報酬'
                  ? 'モンスター'
                  : '入手先';
        const location = (row: SourceRow) => (
          <>
            {row.monsterId !== undefined ? (
              <Anchor component={Link} to={`/monsters/${row.monsterId}`}>
                {row.location}
              </Anchor>
            ) : row.questId !== undefined ? (
              <Anchor component={Link} to={`/quests/${row.questId}`}>
                {row.location}
              </Anchor>
            ) : (
              row.location
        ...
        if (['剥ぎ取り', '破壊', '報酬', '交換・おすそわけ'].includes(group))
          columns.push({ title: '種類', render: (row) => sourceNames[row.method] ?? row.method });
        if (rows.some((row) => row.material !== undefined))
          columns.push({
            title: group === '焚き火焼き' ? '焼く素材' : '渡す素材',
            render: (row) =>
              row.material ? materialLink(row.material) : row.method === 'おすそわけ' ? '不要' : '不明',
          });
        if (group === '購入' && rows.some((row) => !points(row)))
          columns.push({
            title: '価格',
            numeric: true,
            render: (row) =>
        ...
        if (rows.some((row) => row.amount !== undefined))
          columns.push({ title: '個数', numeric: true, render: (row) => row.amount ?? '不明' });
        if (rows.some((row) => row.chance !== undefined))
          columns.push({
            title: '確率',
            numeric: true,
            render: (row) => (row.chance === undefined ? '不明' : `${row.chance}%`),
          });
        if (rows.some((row) => row.rank !== undefined))
              ...
              <Box className="responsive-table-container">
                <Table className="responsive-table">
                  <Table.Thead>
                    <Table.Tr>
                      {columns.map((column) => (
                        <Table.Th key={column.title} className={column.numeric ? 'numeric-cell' : undefined}>
                          {column.title}
                        </Table.Th>
                      ))}
                    </Table.Tr>
                  ...
                  <Table.Tbody>
                    {rows.map((row, index) => (
                      <Table.Tr key={index}>
                        {columns.map((column) => (
                          <Table.Td key={column.title} className={column.numeric ? 'numeric-cell' : undefined}>
                            {column.render(row)}
                          </Table.Td>
                        ))}
                      </Table.Tr>
