...
export function EquipmentMaterialTable({
  materials,
  itemById,
}: {
      ...
      <Table.Tbody>
        {materials.map((material, index) => (
          <Table.Tr key={`${material.item_id}-${index}`}>
            <Table.Td>
              <MaterialLink itemId={material.item_id} itemById={itemById} />
            </Table.Td>
...
export function EquipmentMaterialGroups({
  groups,
  itemById,
}: {
  groups: { key: string | number; label: ReactNode; materials: EquipmentMaterial[] }[];
  itemById: ItemById;
}) {
  return (
        ...
        <Stack key={group.key} gap={4}>
          ...
          {group.materials.map((material, index) => (
            <Group
              ...
              gap="xs"
            >
              <Text size="sm" style={{ minWidth: 0, flex: 1, overflowWrap: 'anywhere' }}>
                <MaterialLink itemId={material.item_id} itemById={itemById} />
              </Text>
