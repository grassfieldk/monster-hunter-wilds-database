...
export function ItemPage() {
  ...
  const useTarget = (use: (typeof uses)[number]) => {
    if (use.category === '調合') {
  ...
  return (
    <Stack className="page-stack" gap="lg">
      <Box visibleFrom="sm">
        ...
        <FormattedText className="long-description" size="sm" mt="sm" monsterIds={monsterIds} style={{ whiteSpace: 'pre-line' }}>{text(item.descriptions)}</FormattedText>
      </Box>

      {activeSection === 'item-basic' && <section id="item-basic">
        <Box hiddenFrom="sm" mb="md">
          <FormattedText className="long-description" size="sm" monsterIds={monsterIds} style={{ whiteSpace: 'pre-line' }}>{text(item.descriptions)}</FormattedText>
        </Box>
            ...
            <Table className="responsive-table responsive-table--intrinsic">
              ...
              <Table.Tbody>
                {uses.map((use, index) => {
                  ...
                  return <Table.Tr key={`${use.category}-${use.name}-${index}`}>
                    <Table.Td>{use.category}</Table.Td><Table.Td>{target ? <Anchor component={Link} to={target}>{use.name}</Anchor> : use.name}{use.detail ? ` ${use.detail}` : ''}</Table.Td><Table.Td className="numeric-cell">{use.amount}</Table.Td>
                  </Table.Tr>;
                })}
              </Table.Tbody>
            </Table>
