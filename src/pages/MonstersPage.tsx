...
export function MonstersPage() {
  ...
  const sortedMonsters = useMemo(
    () => [...monsters].sort((a, b) => label(a.species).localeCompare(label(b.species), 'ja') || a.game_id - b.game_id),
    [monsters],
  );
    ...
    <Stack className="page-stack" gap="md">
      <Box className="responsive-table-container">
        <Table className="responsive-table" striped highlightOnHover withTableBorder>
          <Table.Thead>
            <Table.Tr>
              <Table.Th>種族</Table.Th>
              <Table.Th>モンスター</Table.Th>
              <Table.Th>二つ名</Table.Th>
            </Table.Tr>
          </Table.Thead>
          <Table.Tbody>
