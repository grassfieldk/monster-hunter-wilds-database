    ...
    <SimpleGrid cols={{ base: 2, sm: 4 }} spacing="sm">
      {stats.map((stat) => (
        <div key={stat.label}>
          ...
          <Text size="sm" fw={500}>{stat.value}</Text>
        </div>
      ))}
    </SimpleGrid>
  );
...
function ArmorDetailStats({ item, seriesName, skillById }: { item: Armor; seriesName: string; skillById: Map<number, Skill> }) {
  return <EquipmentStats stats={[
  ...
  return <Stack className="page-stack" gap="lg">
    ...
    <Stack hiddenFrom="sm" gap="sm">
      {pieces.map((item) => <Stack key={item.game_id} gap={4}>
        ...
        <Text size="sm" c="dimmed" style={{ overflowWrap: 'anywhere' }}>スキル: {formatEquipmentSkills(item.skills, skillById)}</Text>
      </Stack>)}
    </Stack>
...
function EquipmentDetail() {
  ...
  return <Stack className="page-stack" gap="lg">
    ...
    {kind === 'weapons' && (() => {
      ...
      return <><WeaponDetailStats item={item} skillById={skillById} /><EquipmentRecipeSection recipes={recipes} itemById={itemById} />{hasTree && <Stack gap="xs"><Text size="sm" fw={600}>派生</Text><WeaponTreeView currentId={item.game_id} weapons={weapons} weaponTrees={weaponTrees} /></Stack>}</>;
    })()}
    {kind === 'armor' && (() => {
      const item = equipment as Armor;
      const series = armorSeries.find((entry) => entry.game_id === item.series_id);
      const upgradeMaterials = armorUpgradeRecipes.find((entry) => entry.series_id === item.series_id)?.materials ?? [];
      return <>
        ...
        {upgradeMaterials.length > 0 && <Stack gap="xs"><Text size="sm" fw={600}>強化素材</Text><EquipmentMaterialTable materials={upgradeMaterials} itemById={itemById} /></Stack>}
      </>;
    })()}
    {kind === 'amulets' && <>
