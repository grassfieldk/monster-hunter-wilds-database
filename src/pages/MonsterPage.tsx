    ...
    <Stack className="page-stack" gap="lg">
      ...
      {activeSection === 'monster-basic' && <section id="monster-basic">
        <Stack>
          ...
          <SimpleGrid cols={{ base: 2, sm: 4 }} spacing={{ base: 'xs', sm: 'sm' }}>
            <div><Text size="sm" c="dimmed">基礎体力</Text><Text size="sm" fw={500}>{monster.base_health?.toLocaleString('ja-JP') ?? '不明'}</Text></div>
      ...
      {activeSection === 'monster-hitzones' && <section id="monster-hitzones">
        <Group justify="flex-end">
          <Button variant="subtle" size="sm" leftSection={<IconHelpCircle size={16} />} aria-label="表の見方" title="表の見方" onClick={openHelp}>
            表の見方
          </Button>
        </Group>
        <Box className="responsive-table-container">
