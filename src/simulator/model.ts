...
export function isVirtualWeapon(item: unknown): item is VirtualWeapon {
  return typeof item === 'object' && item !== null && 'sourceWeaponId' in item && typeof item.sourceWeaponId === 'string';
}
...
export type ArtianBonus = { id: number; name: string; normalMax: number; gogmaMax: number; attack: number[]; affinity: number[]; attribute: number[]; sharpness: number[] };
export type ArtianSkillData = { weaponIds: string[]; skillPairs: { groupSkillId: number; seriesSkillId: number }[]; bonuses: ArtianBonus[] };
export type SkillTarget = { id: number; level: number };
export type SortMode = 'slots' | 'defense';
export type GearData = { weapons: Weapon[]; armor: Armor[]; amulets: Amulet[]; decorations: Decoration[]; maxSkillLevels: Record<number, number>; skillNames: Record<number, string>; randomAmulets: RandomAmuletData; artianSkills: ArtianSkillData };
export type BuildSummary = { skills: Map<number, number>; defense: number; resistances: number[]; freeSlots: number[]; attack: number | null; affinity: number | null; attributeValue: number | null; sharpnessBonus: number };
...
export function validBuild(value: unknown): value is Build {
  ...
  if (build.weaponBonuses !== undefined && (!Array.isArray(build.weaponBonuses) || build.weaponBonuses.length > 5 || !build.weaponBonuses.every((id) => Number.isSafeInteger(id)))) return false;
  return Object.entries(build.decorations).every(([slot, ids]) => equipmentSlots.includes(slot as EquipmentSlot)
    && Array.isArray(ids) && ids.length <= 3 && ids.every((id) => id === null || Number.isSafeInteger(id)));
}
...
function virtualAmulet(id: string, data: GearData): VirtualAmulet | null {
  ...
  const skills = parts.slice(2).map((part, index) => {
    ...
    return data.randomAmulets.groups[combo.groups[index]]?.some((option) => option.skill_id === skill.skill_id && option.level === skill.level) ? skill : null;
  });
  if (skills.some((skill) => !skill) || new Set(skills.map((skill) => skill?.skill_id)).size !== skills.length) return null;
  const entries = skills as EquipmentSkill[];
  return {
    game_id: id, amulet_type: -1, level: 0, rarity: combo.rarity, price: 0,
    names: { ja: `鑑定護石 ${entries.map((skill) => `${data.skillNames[skill.skill_id] ?? skill.skill_id} Lv ${skill.level}`).join('・')}` },
    descriptions: { ja: '' }, skills: entries,
...
function virtualWeapon(id: string, weapons: Map<string, Weapon>, data: GearData): VirtualWeapon | null {
  ...
  if (!data.artianSkills.skillPairs.some((entry) => entry.groupSkillId === groupSkillId && entry.seriesSkillId === seriesSkillId)) return null;
  return {
    ...weapon, game_id: id, sourceWeaponId,
...
export function summarizeBuild(build: Build, data: GearData): BuildSummary {
  ...
  const resistances = Array.from({ length: 5 }, (_, index) => selected.armor.reduce((sum, item) => sum + (item?.resistances[index] ?? 0), 0));
  ...
  if (isVirtualWeapon(selected.weapon) && build.weaponBonuses?.length) {
    const typeIndex = ['LongSword', 'ShortSword', 'TwinSword', 'Tachi', 'Hammer', 'Whistle', 'Lance', 'GunLance', 'SlashAxe', 'ChargeAxe', 'Rod', 'Bow', 'HeavyBowgun', 'LightBowgun'].indexOf(selected.weapon.weapon_type);
  ...
  for (const slot of equipmentSlots) {
    const item = slot === 'weapon' ? selected.weapon : slot === 'amulet' ? selected.amulet : selected.armor[armorSlots.indexOf(slot)];
    if (!item) continue;
  ...
  return { skills, defense, resistances, freeSlots: freeSlots.sort((a, b) => b - a), attack, affinity, attributeValue, sharpnessBonus };
}
