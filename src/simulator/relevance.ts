...
const shots = new Set(['通常弾・通常矢強化', '貫通弾・竜の矢強化', '散弾・剛射強化', '弾導強化', '特殊射撃強化', 'ファーストショット', 'フォースショット']);
const guardsOnly = new Set(['ガード性能', 'ガード強化', '攻めの守勢']);
const elements = new Map([
  ['火属性攻撃強化', 1], ['水属性攻撃強化', 2], ['氷属性攻撃強化', 3],
  ['雷属性攻撃強化', 4], ['龍属性攻撃強化', 5], ['毒属性強化', 6],
  ['麻痺属性強化', 7], ['睡眠属性強化', 8], ['爆破属性強化', 9],
  ['毒ダメージ強化', 6],
]);
const priority = new Set([
  '攻撃', '見切り', '超会心', '弱点特効', '挑戦者', '連撃', '精霊の加護',
  '体術', 'スタミナ急速回復', '回避性能', '回避距離ＵＰ', '業物', '達人芸',
]);
const specialized = new Set([
  '笛吹き名人', '高速変形', '砲弾装填', '砲術', '溜打強化', '速射強化', ...bowOnly, ...shots,
]);
const fieldSkills = new Set([
  '植生学', '地質学', '昆虫標本の達人', 'ハンター生活', 'クライマー',
]);

export function restrictedSkill(name: string): boolean {
  return name === '笛吹き名人' || name === '高速変形' || name === '砲弾装填'
    || name === '砲術' || name === '溜打強化'
    || name === '速射強化' || sharpness.has(name) || bowOnly.has(name)
    || shots.has(name) || guardsOnly.has(name) || elements.has(name)
    || name === '会心撃【属性】' || name === '会心撃【特殊】';
}
...
export function skillUsable(name: string, weapon: Weapon): boolean {
  ...
  if (name === '砲術') return type === 'GunLance' || type === 'ChargeAxe' || type === 'LightBowgun' || type === 'HeavyBowgun';
  ...
  const attributes = [weapon.attribute_value > 0 ? weapon.attribute : 0, weapon.sub_attribute_value > 0 ? weapon.sub_attribute : 0];
...
export function utilityForSkills(skills: EquipmentSkill[], targetIds: Set<number>, names: Record<number, string>): number {
  return skills.reduce((sum, skill) => sum + (targetIds.has(skill.skill_id) ? 0 : skill.level * skillUtility(names[skill.skill_id] ?? '')), 0);
}
